using System;
using UnityEngine;

namespace PixelCrew.Model.Data.Properties
{
    [Serializable] //тога появится в юнити
    public class FloatPersistentProperty: PrefsPersistentProperty<float> //передаём родителю конкретный тип
    {
        public FloatPersistentProperty(float defaultValue, string key) : base(defaultValue, key)
        {
            Init(); //проинициализируем проперти
        }

        protected override void Write(float value)
        {
            PlayerPrefs.SetFloat(Key, value);
            PlayerPrefs.Save();
        }

        protected override float Read(float defaultValue)
        {
            return PlayerPrefs.GetFloat(Key, defaultValue);
        }
    }
}