...
namespace PixelCrew.UI.Windows.PlayerStats
{
    public class PlayerStatsWindow: AnimatedWindow
    {
        ...
        protected override void Start()
        {
            ...
            _trash.Retain(_session.StatsModel.Subscribe(OnStatsChanged)); //идём в сессию, находим модель и подписываемся на неё
        ...
        private void OnUpgrade()
        {
            ...
            _session.StatsModel.LevelUp(selected); //попытаемся апгрейдить
        }

        private void OnStatsChanged()
        {
            var stats = DefsFacade.I.Player.Stats; //нужно получить список всех статов
            _dataGroup.SetData(stats);

            var selected = _session.StatsModel.InterfaceSelectedStat.Value; //текущий выбранный параметр героя
            var nextLevel = _session.StatsModel.GetCurrentLevel(selected) + 1;
            var def = _session.StatsModel.GetLevelDef(selected, nextLevel);
            //if (_price == null) return; на всякий случай проверка на нал
            _price.SetData(def.Price); //обновляем цену на текущ выбранный параметр
