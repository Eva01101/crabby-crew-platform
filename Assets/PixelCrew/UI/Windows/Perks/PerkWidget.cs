...
namespace PixelCrew.UI.Windows.Perks
{
    public class PerkWidget: MonoBehaviour, IItemRenderer<PerkDef>
    {
        ...
        private PerkDef _data; //данные, котор установили

        private void Start() //на старте нужно забрать сессию, так как до этого она нам не доступна
        {
            _session = FindObjectOfType<GameSession>(); 
            UpdateView();
        }

        public void SetData(PerkDef data, int index) //сет дата может вызваться раньше старта
        //если мы будем заполнять данные на awake
        {
            _data = data;
